...
from tasks.KekkaiActivationSelf.assets import KekkaiActivationSelfAssets
from tasks.KekkaiActivationSelf.utils import parse_rule
from tasks.KekkaiActivationSelf.config import ActivationConfig
from tasks.Utils.config_enum import ShikigamiClass
from tasks.GameUi.page import page_main, page_guild
from tasks.KekkaiActivationSelf.config import CardType
...
class ScriptTask(KU, KekkaiActivationSelfAssets):
    ...
    def order_targets(self) -> ImageGrid:
        rule = self.config.kekkai_activation_self.activation_config.card_type
    ...
    def run_activation(self, _config: ActivationConfig) -> bool:
        ...
        while 1:
            ...
            if card_status and card_effect:
                ...
                self.set_next_run("KekkaiActivationSelf", target=interval+datetime.now())
                return False
            # 如果已经选中这张卡了， 那就激活这张卡
            ...
            if self.appear(self.I_A_EMPTY):
                while 1:
                    ...
                    if not self.appear(self.I_A_EMPTY):
                        self.config.kekkai_activation_self.activation_config.card_not_found_count = 0
                        self.config.save()
                        message = f'✅ 确认挂卡: {rule}'
        ...
        while 1:
            ...
            for result in filtered_results:
                ...
                if numbers:
                    ...
                    # 识别星级 (如 "5星" 或文本中其他数字)
                    star_match = re.search(r'([1-6])\s*星', text)
                    if star_match:
    ...
    def _card_not_found(self):
        # 获取配置引用
        activation_config = self.config.kekkai_activation_self.activation_config
        # 多少分钟后重试
        retry_minutes = 180
        retry_count = 3
        # 递增未找到卡的计数器
        ...
        # 保存配置并设置下次执行
        self.config.save()
        self.set_next_run("KekkaiActivationSelf", target=next_run)
        raise TaskEnd
