...
class ActivationSchedulerSelf(Scheduler):
...
class ActivationConfigSelf(BaseModel):
    card_type: CardType = Field(default=CardType.TAIKO, description='card_rule_help')
    star_priority: str = Field(default='6>5>4>3', description='star_priority_help')
...
class KekkaiActivationSelf(ConfigBase):
    scheduler: ActivationSchedulerSelf = Field(default_factory=ActivationSchedulerSelf)
    activation_config: ActivationConfigSelf = Field(default_factory=ActivationConfigSelf)


ActivationConfig = ActivationConfigSelf
ActivationScheduler = ActivationSchedulerSelf
